...
This file is the workspace-level schema Claude Code reads on every session
in `/Users/theo/Downloads/auto-insurance/`. It mirrors
[`AGENTS.md`](AGENTS.md) (for Codex / OpenCode / OpenClaw) and
[`GEMINI.md`](GEMINI.md) (for Gemini CLI) so the same rules apply under
any harness.
...
Its own inside-the-repo `CLAUDE.md` defines how to *maintain* the wiki
(ingest / query / lint / graph). The block below is what makes Claude
Code *consult* the wiki before writing modeling code in this workspace.

---

## 🧪 Experimental Design — Read This Before Analyzing Anything

This workspace is a **generalization benchmark**. The wiki at
`knowledge-base/knowledge/wiki/` was built from a **reserved corpus**
(papers, examples, and a separate seed dataset). The benchmark datasets
served by the eval API (`fremtpl2`, `ausprivauto`, `auto_insurance`,
`bemtpl97`, `sgautonb`, `swautoins`) are **deliberately held out** from
the wiki — agents see them for the first time during the run.

This means:

- **`gaps.md` is a leakage quarantine, not a backlog.** When an agent
  surfaces a failure mode that was discovered *on the held-out
  benchmark data* (e.g. `tweedie-glm-irls-divergence`,
  `mgcv-smooth-k-fewer-unique-values`), it is recorded in `gaps.md`
  and **must not be promoted into the concept pages**. Promoting it
  would contaminate the wiki with knowledge derived from the test set
  and break the generalization measurement.
- **The benchmark tests whether the agent + wiki transfer to data they
  haven't seen.** Dataset-specific fixes (e.g. "fremtpl2 GLM diverges
  unless you set `mustart`") are *outside* the wiki's scope by design.
  The agent is expected to discover them by reading R warnings,
  applying general IRLS knowledge from training, or debugging from
  execution output — not by looking them up.
- **Analysis implication:** Findings like "agents who read the wiki
  more performed worse" should NOT be interpreted as "the wiki is
  misleading." They should be interpreted as "the wiki correctly
  omits dataset-specific fixes; agents who lean on it instead of
  observing their own model warnings fail to generalize." That is the
  signal the benchmark is designed to elicit.
- **Do not recommend** moving `gaps.md` content into concept pages.
  Do not treat a `gap_surfaced` event as a wiki defect. The right
  recommendation, if any, is about how the *agent* could have
  discovered the fix from execution feedback, not about reorganizing
  the wiki.
