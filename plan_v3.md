# Auto Insurance Pricing Model Benchmark — Agent Execution Plan (v3)
...
6. Fits on **train**, tunes on **eval** per the wiki's tuning guidance for each model (no tuning for GLM).
7. Predicts on **eval**, computes `eval_gini` via `lib/gini.R`.
...
`*_eval_gini` = agent-side values. `*_test_gini` = grader-side values from the orchestrator's grader pass (Section 8). `se_*` is the **standard error of the mean** across successful trials (`sd / sqrt(n_successful)`), not the raw standard deviation across seeds — it is the right unit for comparing methods.
