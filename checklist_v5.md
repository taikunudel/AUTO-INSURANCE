...
nobody can read `1.7`, `2A.4`, `9.5` and remember what those mean. Every audit
...
Companion to [plan_v5.md](plan_v5.md). A complete enumeration of observable, verifiable requirements with checking methods. Use this as a go/no-go review of any run. **(v5: Tweedie GLM removed — 4 models: GAM, GrpLasso, GrpNet, TDboost.)**

### Phase 0 — Workspace Setup & Isolation

| # | Requirement | How to verify |
|---|---|---|
| 0.1 | Workspace folder named `run-<harness>-<model>-<thinking>-run<X>-wiki<MMDD>-<YYYYMMDD-HHMMSS>` — all lowercase, hyphens only, **no underscores**; `<harness>` ∈ {`claudecode`,`codex`,`openclaw`,`gemini`}; OpenRouter-served models carry an `or-` prefix on `<model>` (e.g. `or-gemini31pro`), direct/vLLM models do not; `run<X>` is the run index (e.g. `run1`), `wiki<MMDD>` is the wiki snapshot (e.g. `wiki0531`) | `basename $PWD` matches `^run-(claudecode\|codex\|openclaw\|gemini)-(or-)?[a-z0-9.-]+-run[0-9]+-wiki[0-9]{4}-[0-9]{8}-[0-9]{6}$` and contains no `_` |
...
For each of the 4 models the wiki has both a concept page and a runnable
...
|---|---|---|---|---|
| 2A.1 | 01 Tweedie GAM | `concepts/GeneralizedAdditiveModels.md` | `examples/wood-2011-gam-reml.R` | Transcript shows ≥1 read; `procedures/01_tweedie_gam.R` uses `mgcv::Tweedie(p, link=power(0))`, `method="REML"`, `select=TRUE`, factors outside `s()` |
| 2A.2 | 02 GrpLasso | `concepts/GroupedElasticNet.md` | `examples/qian-2016-hdtweedie.R` | Transcript shows ≥1 read; `procedures/02_grplasso.R` uses HDtweedie (or equivalent) with `alpha=1.0` |
| 2A.3 | 03 GrpNet | `concepts/GroupedElasticNet.md` | `examples/qian-2016-hdtweedie.R` | Transcript shows ≥1 read; `procedures/03_grpnet.R` uses grouped elastic net with `alpha ∈ (0,1)` |
| 2A.4 | 04 TDboost | `concepts/GradientTreeBoosting.md` | `examples/yang-2016-tdboost.R` | Transcript shows ≥1 read; `procedures/04_tdboost.R` uses `distribution=list(name="EDM", alpha=p)` and picks `best_iter` from `TDboost.perf(method="cv")` |

#### Phase 2B — Cross-cutting concept pages

These pages apply across all 4 models and the evaluation pipeline.
...
| 2B.2 | `concepts/GiniIndex.md` | Ordered Lorenz, not classical Gini; report on 0–1 fraction scale | Read evidence + `lib/gini.R` implements ordered Gini per Frees–Meyers–Cummings and returns a 0–1 fraction |
...
| 2B.4 | `concepts/LeakageAudit.md` | Required protocol when any trial's gini exceeds 0.5 (0–1 scale) | Read evidence + code path (in orchestrator or aggregator) triggers an audit when threshold exceeded |
...
|---|---|---|
| 2C.1 | GAM: `mgcv::Tweedie(p, link=power(0))` — NOT `link.power=0` (that's the `statmod::tweedie` spelling) | `procedures/01_tweedie_gam.R` |
| 2C.2 | GAM: `method="REML"`, never the default `GCV.Cp` | `procedures/01_tweedie_gam.R` |
| 2C.3 | GAM: factor predictors enter directly, NOT inside `s()`; cap each smooth's `k` at the number of unique covariate values (`k = min(desired, n_unique-1)`) so low-cardinality columns don't crash | `procedures/01_tweedie_gam.R` |
| 2C.4 | HDtweedie design matrix built with `~ ... - 1` (no intercept column) | `procedures/02_grplasso.R`, `procedures/03_grpnet.R`, or `lib/data_loader.R::build_grouped_design` |
| 2C.5 | HDtweedie `predict(..., s = "lambda.min")` explicitly | `procedures/02_grplasso.R`, `procedures/03_grpnet.R` |
| 2C.6 | TDboost: predict at `best_iter` from `TDboost.perf(method="cv")`, NEVER at `fit$n.trees` | `procedures/04_tdboost.R` |
...
| 2G.2 | Leakage audit triggered when any trial's gini exceeds 0.5 (0–1 scale) | `concepts/LeakageAudit.md` read AND the orchestrator/aggregator has a code path that fires the audit when the threshold is crossed |
...
| 3.6 | Grouped design matrix produced for `grplasso`/`grpnet` only; not for `gam`/`tdboost` | Procedures `02`/`03` build groups; `01`/`04` do not |
...
| # | Requirement | How to verify |
|---|---|---|
| 4.1 | All 4 R packages loadable | Preflight `library()` succeeds |
| 4.2 | Tweedie power p = 1.7 used by every model | Grep all 4 procedures for `1.7` in family/distribution args |
| 4.3 | Each model's predict closure produces non-NA, finite predictions on test | Run any saved closure on raw test rows; output is numeric and complete |
...
| 5.2 | `set.seed(1000 * RUN_INDEX + trial)` called early, with `RUN_INDEX` read from the env (default 1) | Grep all 4 |
| 5.3 | Reads `EVAL_API_URL_RESOLVED` (NOT `EVAL_API_URL`) | Grep |
| 5.4 | Reads `EVAL_DATASET` (and `RUN_INDEX`, default 1) | Grep |
| 5.5 | Fits on train, tunes on eval | Code flow |
| 5.6 | Computes `eval_gini` (non-NA on success) on the **0–1 fraction** scale | Open trial CSV; `eval_gini` is finite numeric in [-1, 1] |
...
| # | Requirement | How to verify |
|---|---|---|
| 7.1 | `lib/gini.R` exists and produces non-NA, numeric `eval_gini` on the **0–1 fraction** scale (cplm's 0–100 output ÷ 100) | Open trial CSVs; `eval_gini` is finite and in fraction units |
| 7.2 | If any trial's `eval_gini` > 0.5 (0–1 scale): leakage audit triggered (consult wiki if present) | Transcript shows audit reference when threshold exceeded |
...
| 8.3 | Complete datasets (4×10 = 40 OK trials) preserved; incomplete ones wiped | Re-run on partial workspace: complete dataset kept, partial wiped |
| 8.4 | Round-robin: all 4 models for round k before any model starts round k+1 | Log file timestamps show all 4 models for round 01 finish before round 02 begins |
| 8.5 | Trials with `status == OK` are skipped on re-run | Re-run after a successful trial; orchestrator reports `[SKIP]` or equivalent |
| 8.6 | Each trial subprocess sees env vars `EVAL_API_URL_RESOLVED`, `EVAL_DATASET`, and `RUN_INDEX` | Inspect R log; values used in the subprocess |
...
| 8.8 | After each dataset's 40 trials, grader pass runs and writes `results/<ds>/grader_scores/<model>_trial_<NN>.json` | File structure populated before next dataset starts |
...
| 10.2 | Per-dataset `summary.csv` written (4 rows, one per model) | `ls results/<ds>/summary.csv`; row count = 4 |
| 10.3 | Top-level `results/summary.csv` written | Row count = N_datasets × 4 |
...
| 11.2 | **H2:** All 4 R packages load | Preflight log |
| 11.3 | **H3:** API reachable; target dataset metric == "gini" | Preflight log |
| 11.4 | **H4:** Smoke fit (GAM, trial 1) succeeds end-to-end | Preflight log |
...
| 12.2 | Each model: 10 trial CSVs | `find results/ -name "trial_*.csv" \| wc -l` ≥ 40 per dataset |
