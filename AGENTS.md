...
(Codex, OpenCode, OpenClaw, etc.) operating inside
`/Users/theo/Downloads/auto-insurance/`. Claude Code reads
[`CLAUDE.md`](CLAUDE.md) and Gemini CLI reads [`GEMINI.md`](GEMINI.md) —
those files mirror the rules below so the same workspace works under any
harness.
...
Its own inside-the-repo `AGENTS.md` defines how to *maintain* the wiki
(ingest / query / lint / graph). The block below is what makes every
agent in this workspace *consult* the wiki before writing modeling code.
